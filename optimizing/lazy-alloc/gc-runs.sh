#!/usr/bin/env bash
# gc-runs.sh — sweep -Zmiri-retag-gc over 50k/100k/250k for all crates in cwd
set -euo pipefail
# ── Config ────────────────────────────────────────────────────────────────────
...
# Base flags shared by every run (no retag-gc here; injected per-run below)
...
# The three retag-gc thresholds to sweep
RETAG_GC_VALUES=(5000 1000 500 100)
CRATES_ROOT="$(pwd)"
CSV="$(pwd)/results.csv"
...
RETAG_GC_VALUES=(${RETAG_GC_VALUES[*]})
...
echo "[$(date -u)] Starting sweep: ${#SIF_NAMES[@]} SIFs × ${#RETAG_GC_VALUES[@]} retag-gc values × ${#CRATE_DIRS[@]} crates"
...
for SIF_NAME in "${SIF_NAMES[@]}"; do
    ...
    for GC_VAL in "${RETAG_GC_VALUES[@]}"; do
        EFFECTIVE_MIRIFLAGS="${BASE_MIRIFLAGS} -Zmiri-retag-gc=${GC_VAL}"

        echo ""
        echo "════════════════════════════════════════"
        echo "[$(date -u)] retag-gc=${GC_VAL}  MIRIFLAGS: $EFFECTIVE_MIRIFLAGS"
        echo "════════════════════════════════════════"
...
echo "Submitted retag-gc sweep → job $JOB_ID"
echo ""
