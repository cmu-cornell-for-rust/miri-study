#!/bin/bash

# default run
valgrind --tool=callgrind --trace-children=yes --trace-children-skip=*/rustc --trace-children-skip-by-arg='--crate-type' cargo test

# run with miri
export MIRIFLAGS="-Zmiri-disable-data-race-detector -Zmiri-disable-validation"
valgrind --tool=callgrind \
    --trace-children=yes \
    --trace-children-skip=*/rustc \
    --trace-children-skip-by-arg='--crate-type','-vV','--print','--format-version','--error-format=json' \
    --callgrind-out-file="callgrind.out.miri.%p"  \
    cargo miri test

# run with treeborrows miri
export MIRIFLAGS="-Zmiri-disable-data-race-detector -Zmiri-disable-validation -Zmiri-tree-borrows"
valgrind --tool=callgrind \
    --trace-children=yes \
    --trace-children-skip=*/rustc \
    --trace-children-skip-by-arg='--crate-type','-vV','--print','--format-version','--error-format=json' \
    --callgrind-out-file="callgrind.out.miri-tree.%p"  \
    cargo miri test